...
% 120 subjects     51 control                 69 pathological
% length(SUB)      sum(ClinicalStatus==1)     sum(ClinicalStatus==0)

% Neurofeedback Performance Scores
figure;
E = [ sample_sd_DMN_NF_UP_2ndHalf/sqrt(length(SUB)) control_sd_DMN_NF_UP_2ndHalf/sqrt(sum(ClinicalStatus==1)) path_sd_DMN_NF_UP_2ndHalf/sqrt(sum(ClinicalStatus==0));...
      sample_sd_DMN_NF_DN_2ndHalf/sqrt(length(SUB)) control_sd_DMN_NF_DN_2ndHalf/sqrt(sum(ClinicalStatus==1)) path_sd_DMN_NF_DN_2ndHalf/sqrt(sum(ClinicalStatus==0));...
      sample_sd_DMN_NF_2ndHalf/sqrt(length(SUB))    control_sd_DMN_NF_2ndHalf/sqrt(sum(ClinicalStatus==1))    path_sd_DMN_NF_2ndHalf/sqrt(sum(ClinicalStatus==0))];
...
figure;
E = [ sample_sd_DMN_NF_UP_learning/sqrt(length(SUB)) control_sd_DMN_NF_UP_learning/sqrt(sum(ClinicalStatus==1)) path_sd_DMN_NF_UP_learning/sqrt(sum(ClinicalStatus==0));...
      sample_sd_DMN_NF_DN_learning/sqrt(length(SUB)) control_sd_DMN_NF_DN_learning/sqrt(sum(ClinicalStatus==1)) path_sd_DMN_NF_DN_learning/sqrt(sum(ClinicalStatus==0));...
      sample_sd_DMN_NF_learning/sqrt(length(SUB))    control_sd_DMN_NF_learning/sqrt(sum(ClinicalStatus==1))    path_sd_DMN_NF_learning/sqrt(sum(ClinicalStatus==0))];
